...
if __name__ == "__main__":
    ...
    low_cut_hz = 0
    test_subject_id = 1
    train_subject_ids = np.delete(np.arange(1,15,1),test_subject_id-1)
    ...
    train_split_samples = cross_train_set_X.shape[0]
    valid_split_samples = cross_valid_set_X.shape[0]

    train_filename =  os.path.join(data_folder, 'train/{:d}.mat'.format(test_subject_id))
    test_filename =  os.path.join(data_folder, 'test/{:d}.mat'.format(test_subject_id))
    #Create the dataset
    train_set, valid_set, test_set = load_train_valid_test(train_filename=train_filename,test_filename=test_filename,low_cut_hz=low_cut_hz)

    subject_train = train_set
    subject_valid = valid_set
    subject_test = test_set
    ...
    with brain_eeg.as_default():
        ...
        with tf.device(pd['device_name']):
            ...
            with tf.variable_scope("eeg_network" , reuse=tf.AUTO_REUSE):
                ...
                dense1 = tf.keras.layers.Dense(4,activation=tf.keras.activations.softmax,kernel_regularizer=tf.keras.regularizers.l2(l=0.01))

                conv1_output = (train_set_x)
                elu1_output = conv1_output
            ...
            with tf.variable_scope("eeg_network" , reuse=tf.AUTO_REUSE):
                ...
                elu1_test_output = (conv1_test_output)
    ...
    with tf.Session(graph=brain_eeg,config=tf.ConfigProto(log_device_placement=True,allow_soft_placement=True)) as sess:
        ...
        max_test_acc = 0
        max_acc_epoch = 0
        #Train on all except test subject , save best model

        for epoch in range(pd["iterations"]):
            #Initialize train and test iterators
            sess.run([train_iter.initializer,test_iter.initializer], feed_dict={ train_placeholder_x: cross_train_set_X, train_placeholder_y: cross_train_set_y,test_placeholder_x: cross_valid_set_X, test_placeholder_y:cross_valid_set_y})
            ...
            while True:
                try:
                    ...
                    sess.run(train_iter.initializer, feed_dict={ train_placeholder_x: cross_train_set_X, train_placeholder_y: cross_train_set_y})
                    ...
                    while True:
                        try:
                            ...
                            if(epoch_test_accuracy >= max_test_acc):
                                max_test_acc = epoch_test_accuracy
                                save_path = saver.save(sess, "./tmp/model_pretrain" + str(epoch) + ".ckpt")
                                max_acc_epoch = epoch
            ...
            print("Test Accuracy : " ,epoch_test_accuracy/valid_split_samples)


        #Train on the specific test subject
        for epoch in range(pd["iterations"]):
            #Initialize best model
            saver.restore(sess, "/tmp/model_pretrain"+str(max_acc_epoch)+".ckpt")
    
            #Initialize train and test iterators
            sess.run([train_iter.initializer,test_iter.initializer], feed_dict={ train_placeholder_x: subject_train.X, train_placeholder_y: subject_train.y,test_placeholder_x: subject_valid.X, test_placeholder_y:subject_valid.y})
