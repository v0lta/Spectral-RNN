...
if __name__ == "__main__":
    ...
    low_cut_hz = 0
    test_subject_id = 3
    #test_subject_id = int(sys.argv[1])
    ...
    subject_train_X = np.concatenate([train_set.X , valid_set.X])
    subject_train_y = np.concatenate([train_set.y , valid_set.y])
    subject_test_X = test_set.X
    subject_test_y = test_set.y

    train_split_samples = subject_train_X.shape[0]
    test_split_samples = subject_test_X.shape[0]
    ...
    with tf.Session(graph=brain_eeg,config=tf.ConfigProto(log_device_placement=True,allow_soft_placement=True)) as sess:
        ...
        for epoch in range(pd["iterations"]):
            ...
            sess.run([train_iter.initializer,test_iter.initializer], feed_dict={ train_placeholder_x: subject_train_X, train_placeholder_y: subject_train_y,test_placeholder_x: subject_test_X, test_placeholder_y:subject_test_y})
            ...
            while True:
                try:
                    ...
                    sess.run(train_iter.initializer, feed_dict={ train_placeholder_x: subject_train_X, train_placeholder_y: subject_train_y})
            ...
            max_test_acc = epoch_test_accuracy/test_split_samples

        print("Max Test Accuracy for {} is {}".format(test_subject_id, max_test_acc))
