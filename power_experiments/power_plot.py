...
if 1:
    ...
    plot_path = '/home/moritz/uni/fourier-prediction/log/cvpr_workshop_power_pred4/'
    logs = return_logs(plot_path, window_size, vtag='mse_net_test')
    stop_at = len(logs[0][0][0])
    plt.plot(logs[4][0][0][:stop_at], logs[4][0][1][:stop_at], label='fft-gru')
    plt.plot(logs[2][0][0][:stop_at], logs[2][0][1][:stop_at], label='reshape-gru')
    plt.plot(logs[1][0][0][:stop_at], logs[1][0][1][:stop_at], label='fft-gru-lowpass')
    plt.plot(logs[0][0][0][:stop_at], logs[0][0][1][:stop_at], label='reshape-gru-down')
