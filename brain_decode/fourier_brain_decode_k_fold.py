...
if __name__ == "__main__":
    ...
    if(pd["data_augmentation"]):
        train_set_temp = np.transpose(np.concatenate([train_set.X,valid_set.X]),axes=[0,2,1])
        train_set_scaled = []
        train_set_permuted = []
        for i in range(train_set_temp.shape[0]):
            train_set_scaled.append(np.expand_dims(DA_Scaling(train_set_temp[i]),axis=0))
            train_set_permuted.append(np.expand_dims(DA_Permutation(train_set_temp[i]),axis=0))
        ...
        train_set_X = np.concatenate([np.concatenate([train_set.X,valid_set.X]),train_set_scaled,train_set_permuted])
        train_set_y = np.tile(np.concatenate([train_set.y,valid_set.y]),reps=3)
    ...
    with brain_eeg.as_default():
        ...
        with tf.device(pd['device_name']):
            ...
            train_dataset = tf.data.Dataset.from_tensor_slices((train_placeholder_x, train_placeholder_y)).shuffle(50).batch(pd["batch_size"])
    ...
    with tf.Session(graph=brain_eeg,config=tf.ConfigProto(log_device_placement=True,allow_soft_placement=True)) as sess:
        ...
        for train_index,test_index in zip(train_index_list,test_index_list):
            ...
            for epoch in range(pd["iterations"]):
                ...
                sess.run([train_iter.initializer,test_iter.initializer], feed_dict={ train_placeholder_x: train_split_X, train_placeholder_y: train_split_y,test_placeholder_x: test_split_X, test_placeholder_y:test_split_y})
                ...
                while True:
                    try:
                        ...
                        sess.run(train_iter.initializer, feed_dict={ train_placeholder_x: train_split_X, train_placeholder_y: train_split_y})
            ...
            split_train_accuracy.append(epoch_train_accuracy/train_split_samples)
            split_test_accuracy.append(epoch_test_accuracy/test_split_samples)
